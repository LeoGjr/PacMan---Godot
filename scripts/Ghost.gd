...
func _process(delta):
	...
	if get_node("../../Control").score < points: return
	var delcont = descont - get_pos()
...
func go_to_next():
	if path == null or path.size() == 0:
		return
	destdisc = path[0]
	descont = destdisc * 32 + Vector2(16, 16)
	path.remove(0)
func calc_move_blue():
	path = []
	var aux = bluedir
...
func _on_Pacman_pac(mode):
	if mode == "die":
		...
		if descont - get_pos() == Vector2(0, 0):
			calc_move()
	elif mode == "reset":
		get_node("Anim").stop()
		set_modulate(Color(1, 1, 1, 1))
